...
class SurfaceFilter:
    ...
    def filter_indices(self, neighbors, ptm_types=None):
        ...
        if ptm_types is not None:
            ptm_types = np.asarray(ptm_types)
            if ptm_types.ndim != 1 or len(ptm_types) != N:
                raise ValueError('ptm_types must be 1D array of length N')
        interior = []
        for i, nbrs in neighbors.items():
            ...
            if len(nbrs) < self.min_neighbors:
                continue
            if ptm_types is not None and ptm_types[i] < 0:
                continue
            interior.append(i)
        return interior

    def filter_data(self, positions, ids, neighbors, ptm_types=None, quaternions=None):
        ...
        if ptm_types is not None:
            ptm_types = np.asarray(ptm_types)
            if ptm_types.ndim != 1 or ptm_types.shape[0] != N:
                raise ValueError('ptm_types must be length N')
        ...
        # filter indices
        interior = self.filter_indices(neighbors, ptm_types)
        M = len(interior)
        # build old->new map
        ...
        if ptm_types is not None:
            result['ptm_types'] = ptm_types[interior]
        if quaternions is not None:
